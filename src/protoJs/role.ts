...
export class RoleViewModel extends pb_1.Message {
    ...
    serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
        ...
        if (this.pIds.length)
            writer.writeRepeatedString(4, this.pIds);
        if (this.sort != 0)
    ...
    static deserialize(bytes: Uint8Array | pb_1.BinaryReader): RoleViewModel {
        const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new RoleViewModel();
...
export class RolePageResponse extends pb_1.Message {
    ...
    get raws() {
        return pb_1.Message.getRepeatedWrapperField(this, RoleViewModel, 2) as RoleViewModel[];
    }
    ...
    serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
        ...
        if (this.raws.length)
            writer.writeRepeatedMessage(2, this.raws, (item: RoleViewModel) => item.serialize(writer));
        if (!w)
            return writer.getResultBuffer();
    }
    static deserialize(bytes: Uint8Array | pb_1.BinaryReader): RolePageResponse {
        const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new RolePageResponse();
        while (reader.nextField()) {
            if (reader.isEndGroup())
                break;
            switch (reader.getFieldNumber()) {
                case 1:
                    ...
                    break;
                case 2:
                    reader.readMessage(message.raws, () => pb_1.Message.addToRepeatedWrapperField(message, 2, RoleViewModel.deserialize(reader), RoleViewModel));
                    break;
                default: reader.skipField();
            }
        }
