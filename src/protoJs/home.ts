...
export class MerchantCell extends pb_1.Message {
  ...
  get promotions() {
    return pb_1.Message.getRepeatedWrapperField(
      this,
      dependency_5.PromotionCell,
      8
    ) as dependency_5.PromotionCell[];
  }
  ...
  static fromObject(data: {
    ...
    promotions?: ReturnType<
      typeof dependency_5.PromotionCell.prototype.toObject
    >[];
  }): MerchantCell {
    ...
    if (data.promotions != null) {
      message.promotions = data.promotions.map((item) =>
        dependency_5.PromotionCell.fromObject(item)
      );
    }
    return message;
  }
  toObject() {
    ...
    if (this.promotions != null) {
      data.promotions = this.promotions.map(
        (item: dependency_5.PromotionCell) => item.toObject()
      );
    }
    return data;
  }
  ...
  serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
    ...
    if (this.rating != 0) writer.writeFloat(4, this.rating);
    if (this.tags.length) writer.writeRepeatedString(5, this.tags);
    if (this.addr.length) writer.writeString(6, this.addr);
    if (this.distance != 0) writer.writeDouble(7, this.distance);
    if (this.promotions.length)
      writer.writeRepeatedMessage(
        8,
        this.promotions,
        (item: dependency_5.PromotionCell) => item.serialize(writer)
      );
    if (!w) return writer.getResultBuffer();
  }
        ...
        case 8:
          reader.readMessage(message.promotions, () =>
            pb_1.Message.addToRepeatedWrapperField(
              message,
              8,
              dependency_5.PromotionCell.deserialize(reader),
              dependency_5.PromotionCell
            )
          );
          break;
