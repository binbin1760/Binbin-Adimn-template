...
export class PostLBSRequest extends pb_1.Message {
  ...
  serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
    ...
    if (this.latitude != 0) writer.writeDouble(1, this.latitude);
    if (this.longitude != 0) writer.writeDouble(2, this.longitude);
...
export class ProfilePetResponse extends pb_1.Message {
  ...
  serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
    const writer = w || new pb_1.BinaryWriter();
    if (this.raws.length)
      writer.writeRepeatedMessage(1, this.raws, (item: ProfilePetCell) =>
  ...
  static deserialize(
    ...
    const reader =
        bytes instanceof pb_1.BinaryReader
          ? bytes
          : new pb_1.BinaryReader(bytes),
      message = new ProfilePetResponse();
    while (reader.nextField()) {
      if (reader.isEndGroup()) break;
      switch (reader.getFieldNumber()) {
        case 1:
          reader.readMessage(message.raws, () =>
            pb_1.Message.addToRepeatedWrapperField(
              message,
              1,
              ProfilePetCell.deserialize(reader),
              ProfilePetCell
            )
          );
          break;
...
export class AddressViewModel extends pb_1.Message {
  ...
  serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
    ...
    if (this.district.length) writer.writeString(7, this.district);
    if (this.title.length) writer.writeString(8, this.title);
    if (this.street.length) writer.writeString(9, this.street);
    if (this.label.length) writer.writeString(10, this.label);
    if (this.isDefault != false) writer.writeBool(11, this.isDefault);
    if (this.gender != dependency_1.Gender._Gender_UNSPECIFIED)
